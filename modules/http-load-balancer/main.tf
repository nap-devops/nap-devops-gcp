...
resource "google_compute_backend_service" "bn" {
  ...
  dynamic "backend" {
    ...
    content {
      group = "projects/${var.project}/zones/${backend.value}-a/instanceGroups/${var.prefix}-group-${backend.value}-a"
    }
  }
  ...
  dynamic "backend" {
    for_each = var.instances_regions
    content {
      group = "projects/${var.project}/zones/${backend.value}-b/instanceGroups/${var.prefix}-group-${backend.value}-b"
    }
  }
  ...
  dynamic "backend" {
    for_each = var.instances_regions
    content {
      group = "projects/${var.project}/zones/${backend.value}-c/instanceGroups/${var.prefix}-group-${backend.value}-c"
    }
  }
